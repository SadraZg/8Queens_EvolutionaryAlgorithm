...
def main():
    ...
    # If there is no valid answer(which has no threats), return false
...
# Therefore, we only need to check the left side for potential threats
def is_safe(board, row, col):
    # Check left side of row (all the way to the border of board)
    for i in range(col):
        if board[row][i] == 1:
            return False

    # Check upper left side (all the way to the border of board)
    for i, j in zip(range(row, -1, -1), range(col, -1, -1)):
        if board[i][j] == 1:
            return False

    # Check lower left side (all the way to the border of board)
...
def solve(board, col):
    ...
    # Meaning it has a threat in that column
    return False
